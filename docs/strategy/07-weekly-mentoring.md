---
title: "Zolai AI — Weekly Mentoring System"
description: "Structured weekly mentoring sessions with Shwe Yee (Strategic Advisor)"
author: "OpenCode orchestra conductor"
created: 2026-09-18
last_updated: 2026-09-18
status: current
---

# Zolai AI — Weekly Mentoring System

**Mentee:** Peter Pau Sian Lian (@peterlianpi)
**Mentor:** Shwe Yee (Strategic Advisor / Business & Impact Mentor)
**Cadence:** Weekly, 60 minutes
**Platform:** Video call (Zoom/Google Meet) or in-person

---

## Role Boundaries

### Shwe Yee IS:
- **Strategic Advisor** — provides business strategy, impact measurement, grant readiness guidance
- **Business & Impact Mentor** — helps Peter think through commercialization, partnerships, sustainability
- **Accountability Partner** — keeps Peter focused on high-impact work
- **Network Connector** — introduces Peter to relevant contacts

### Shwe Yee is NOT:
- **Co-founder** — unless a separate formal agreement is made (see 08-25-percent-arrangement.md)
- **Technical co-lead** — technical decisions remain with Peter
- **Employee** — this is advisory, not employment
- **Legal counsel** — for legal questions, refer to qualified professionals

### Decision Authority:
- **Peter decides:** All technical, product, and day-to-day operational decisions
- **Shwe Yee advises:** Business strategy, grant applications, partnerships, community engagement
- **Both decide:** Strategic direction (discussed, Peter has final say)
- **Formal agreement needed:** Any equity, revenue sharing, or commercial arrangement

---

## Meeting Structure (6 × 10-min segments)

### Segment 1: Check-in (10 min)
- How are you feeling this week? (1-2 min)
- What went well since last meeting? (3 min)
- What was challenging? (3 min)
- Any blockers or concerns? (3 min)

### Segment 2: OKR Review (10 min)
- Quick scan of OKR dashboard (2 min)
- Which KRs are green/yellow/red? (3 min)
- What evidence have you collected? (3 min)
- Any KRs at risk? (2 min)

### Segment 3: Priority Focus (10 min)
- What are the top 3 priorities this week? (3 min)
- Are these the RIGHT priorities? (3 min)
- What should you STOP doing? (2 min)
- What's the most impactful single action? (2 min)

### Segment 4: Deep Dive (10 min)
- One specific topic per week (rotating):
  - Week 1: Grant strategy / funding landscape
  - Week 2: Business model / revenue streams
  - Week 3: Community engagement / partnerships
  - Week 4: Research / publications / conferences
- Focus on actionable outcomes, not just discussion

### Segment 5: Skill Development (10 min)
- What skill are you developing this week? (2 min)
- What did you learn from the founder OKR activities? (4 min)
- How does this skill development serve the project? (2 min)
- Any learning resources to recommend? (2 min)

### Segment 6: Action Items & Close (10 min)
- Recap action items from this meeting (3 min)
- Confirm next meeting date/time (1 min)
- Rate this session (1-5) (1 min)
- Any final thoughts or questions? (5 min)

---

## Preparation Checklist (Before Each Meeting)

### Peter Must Prepare:
- [ ] OKR dashboard updated with current status
- [ ] Evidence collected for any KRs completed this week
- [ ] Top 3 priorities for next week identified
- [ ] One specific topic for Deep Dive segment
- [ ] Any decisions needing Shwe Yee's input
- [ ] Questions or concerns written down

### Shwe Yee Should Review:
- [ ] Previous meeting action items
- [ ] OKR progress since last meeting
- [ ] Any relevant market/funder news
- [ ] Potential introductions or opportunities

---

## Post-Meeting Template

```markdown
# Mentoring Session — [DATE]

## Attendees
- Peter Pau Sian Lian
- Shwe Yee

## Duration
- [Start time] – [End time]

## Key Discussion Points
1. [Topic 1]
2. [Topic 2]
3. [Topic 3]

## Decisions Made
- [Decision 1]
- [Decision 2]

## Action Items
| # | Action | Owner | Due Date | Status |
|---|--------|-------|----------|--------|
| 1 | [Action] | [Peter/Shwe] | [Date] | [ ] |
| 2 | [Action] | [Peter/Shwe] | [Date] | [ ] |

## OKR Updates
- [Any KRs that moved from red→yellow or yellow→green]

## Next Meeting
- Date: [Date]
- Time: [Time]
- Focus topic: [Topic]

## Session Rating
- Peter: [1-5]
- Shwe Yee: [1-5]
```

---

## Decision Framework

### When Peter Faces a Decision:

1. **Is this a technical decision?**
   - Yes → Peter decides (Shwe Yee provides input only if asked)
   - No → Continue to step 2

2. **Is this a business/strategy decision?**
   - Yes → Discuss with Shwe Yee, Peter decides
   - No → Continue to step 3

3. **Is this a partnership/funder decision?**
   - Yes → Shwe Yee advises, Peter decides
   - No → Peter decides independently

4. **Does this affect the undefined commercial arrangement?** (see stub `08-25-percent-arrangement.md` — no public terms)
   - Yes → Both must agree, document in 08-25-percent-arrangement.md
   - No → Peter decides

### Escalation Rules:
- **If Peter is stuck on a decision for > 1 week:** Escalate to Shwe Yee
- **If Shwe Yee disagrees with Peter's direction:** Shwe Yee states concern, Peter decides
- **If there's a conflict of interest:** Disclose immediately, recuse from decision
- **If a decision requires legal/expert advice:** Refer to qualified professional

---

## Meeting Cadence Options

### Option A: Weekly (Recommended)
- **When:** Every [Day] at [Time]
- **Duration:** 60 minutes
- **Best for:** Active 90-day sprint periods

### Option B: Biweekly
- **When:** Every other [Day] at [Time]
- **Duration:** 60-90 minutes
- **Best for:** Maintenance periods

### Option C: Monthly (Minimum)
- **When:** First [Day] of each month
- **Duration:** 90 minutes
- **Best for:** Strategic review periods

---

## Communication Between Meetings

### Preferred Channels:
- **Urgent questions:** WhatsApp/SMS (response within 24 hours)
- **Non-urgent questions:** Email or GitHub comment (response within 48 hours)
- **Updates:** Weekly email summary (Peter sends to Shwe Yee every Friday)
- **Documents:** GitHub repo (all docs version-controlled)

### Weekly Update Email Template:
```
Subject: Zolai AI Weekly Update — [DATE]

Hi Shwe Yee,

## This Week
- [Key achievement 1]
- [Key achievement 2]
- [Key challenge]

## OKR Status
- Green: [KRs]
- Yellow: [KRs]
- Red: [KRs]

## Next Week Priorities
1. [Priority 1]
2. [Priority 2]
3. [Priority 3]

## Need Your Input On
- [Question/decision needed]

Thanks,
Peter
```

---

## Success Metrics for Mentoring

| Metric | Target | Measurement |
|--------|--------|-------------|
| Meeting attendance | 90%+ | Attendance log |
| Action items completed | 80%+ | Post-meeting tracking |
| OKR progress | Consistent advancement | Monthly scorecard |
| Peter's skill growth | Measurable improvement | Founder OKR scores |
| Strategic decisions made | Timely, high-quality | Decision log |

---

*This mentoring system is reviewed quarterly and adjusted based on needs.*
